...
- [ ] Install latest x64 build to:
  `C:\Program Files\Common Files\VST3\Choroboros Beta.vst3`
- [ ] Optional user mirror:
  `C:\Users\<user>\AppData\Local\Programs\Common\VST3\Choroboros Beta.vst3`
...
- [ ] **Four-file parity (repo):** On a Unix machine or CI, from repo root: `./scripts/verify_factory_json_sync.sh` exits **0** (proves `Assets/defaults_factory_mac.json`, `json_defaults_dump.json`, `linux/linux_factory_defaults.json`, and `windows/windows_factory_defaults.json` are byte-identical). See `docs/FACTORY_DEFAULTS_JSON_SYNC.md`.
- [ ] Run:
  `powershell -ExecutionPolicy Bypass -File .\windows\trace_effective_defaults.ps1`
- [ ] `EffectiveUser` JSON is valid
- [ ] Diff vs repo source (`windows/windows_factory_defaults.json`) is zero changed keys relative to the **same** canonical bytes as mac/linux/dump (not a stale one-off Windows JSON)
...
- [ ] Packaging command executed:
  `powershell -ExecutionPolicy Bypass -File .\windows\package_windows_release.ps1 -Config Release -VersionLabel v2.05`
