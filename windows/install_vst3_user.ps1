...
$vst3Candidates = Get-ChildItem -Path $BuildDir -Recurse -Directory -Filter "Choroboros Beta.vst3" -ErrorAction SilentlyContinue
if ($null -eq $vst3Candidates -or $vst3Candidates.Count -eq 0) {
    throw "No Choroboros Beta.vst3 directory found under '$BuildDir'. Build VST3 first."
}
...
$dest = Join-Path $destRoot "Choroboros Beta.vst3"
