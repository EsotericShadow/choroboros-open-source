...
### Fixed
- **Blue HQ Thiran (reverted aggressive crossing experiment):** Mid-crossfade abort + same-sample chained integer crossings caused hard blend jumps that sounded like bit-crush / heavy aliasing, not chorus. **Thiran only** is back to crossing only when `xfadeCounter <= 0`, and uses **block-constant** centre delay for its inner smoother (per-sample centre ms stays for other cores). See `docs/HIL_V205_A_THIRAN_BLUE_HQ.md`.
- **Blue HQ Thiran (fractional zipper / “distortion” with Color):** The active allpass **one-pole smooths** `a[]` toward the analytic Thiran target (τ **14 ms**); integer handoffs still snap on the new branch. Coefficient smoothing now runs **during** integer crossfades as well (previously skipped for ~6 ms, which froze `frac` on the fade-in path and caused droning zipper). Delay-line smoothing **24 ms**, centre smoother **5 ms**, wet output LP **~9.5 kHz** (Thiran-only).
- **Blue HQ Thiran (mid-xfade floor advance — reverted):** Forcing `xfadeCounter = 0` when `floor(smoothedDelay)` moved before the **6 ms** blend finished was **reverted**; it audibly **reintroduced bitcrush-like / stepped waves** (same failure class as the earlier mid-xfade abort experiment). See `docs/HIL_V205_A_THIRAN_BLUE_HQ.md` attempt log.
- **Blue HQ Thiran (A9/A10 reverted; A11):** **A9** (both channels `lfoBuffer` only) and **A10** (coeff τ 4 ms) **reverted** — user reported **left-only** pulsing / alarm-like crush (likely regression + Thiran still not symmetric in perception). **A11:** Thiran delay line now **reads bulk taps then writes** the current sample into `buffer[writePos]` (strictly causal). See `docs/HIL_V205_A_THIRAN_BLUE_HQ.md`.
- **Blue HQ Thiran (A12 reverted):** A12 (4.5 ms xfade, 30 ms delay smooth, ~8.8 kHz wet LP) was **reverted**; user reported **worse**, steady “chrrrr” artifact. Restored **6 ms** crossfade, **24 ms** delay smoothing, **~9.5 kHz** wet LP. See `docs/HIL_V205_A_THIRAN_BLUE_HQ.md` attempt log A12.
- **Blue HQ Thiran (A14 — xfade scheduling, A13 reverted):** **A13** (clamping `smoothedDelay` for the whole xfade) was **reverted** after reports of **low choppy droning** (fights the LFO). **A14:** queue latest `floor(smoothedDelay)` in `pendingIntHopTarget` while `xfadeCounter`>0 and apply a **catch-up** integer hop when the blend ends (`max(pending, live)` + bucket snap); **during** xfade, fractional Thiran coeff smoothing uses the **fade-in** tap’s `intDelay` with a **clamped** fractional offset so `smoothCoeffsTowardD` is not skipped. See `docs/HIL_V205_A_THIRAN_BLUE_HQ.md` attempt log A14.
- **Depth / centre-delay zippering:** `smoothedCentreDelay` was advanced with `getNextValue()` once per block and `skip(block-1)`, so delay cores saw a staircase. The smoother is now consumed into `centreDelayPerSampleMsBuffer` each sample; Cubic, Lagrange 3/5, Linear, Linear Ensemble, Phase Warp, and Orbit read per-sample centre ms (**Thiran** uses block-constant centre only — see HIL-A note). See `docs/HIL_V205_B_DEPTH_COLOR_ZIPPER.md`.
- **`ChorusDSP::reset` LFO depth staging:** `oscVolume` now initialises from `mapDepthToEngineRange(depth)` to match `prepare()` and the audio-thread path (avoids a raw-depth step on transport reset).
...
### Changed
- **Blue HQ Thiran (rebuilt):** New `ChorusCoreThiran` replaces the archived dual-tap path: **Lagrange-5 delay-line reads** for modulation, **quantised** Thiran `D` with **piecewise-constant** DFII coefficients, and **short sin²/cos² crossfades** only on quant steps (no per-sample coefficient chasing). The superseded implementation remains under `Source/Cores/blue_engine_modern/legacy_inactive/` for reference only (not built).
- **Dev console wet-path isolation:** `probe <0..14>` / `probe list` — `thiranReductionProbe` strips one stage at a time (Thiran 1–4, Blue Focus 5, pre-emphasis 6, output LPF 7, width 8, safety limiter 9, input HPF 10, post-mix peak catch 11, output trim 12, Thiran inner centre snap 13, LFO depth snap 14); `probe 0` clears both NQ+HQ for the current engine colour. Documented in `docs/HIL_V205_A_THIRAN_BLUE_HQ.md`.
- **Default colour smoothing:** `RuntimeTuning::colorSmoothingMs` default raised from 20 ms to 26 ms (still clamped by `applyRuntimeTuning()`).
- **Dev Panel modulation workbench:** Adds **Color** and **Dry/Wet Mix** sliders with the same APVTS mapping as the main macro row (`docs/HIL_V205_C_HQ_NQ_AND_DEVPANEL.md`).
...
- **GitHub Actions (Windows AAX):** Windows AAX build, PACE steps, and AAX in the Windows zip are **deferred** until PACE signing is viable on hosted runners (e.g. Cloud Signing); `docs/GITHUB_ACTIONS_WINDOWS_AAX.md` documents current vs future playbook. Windows CI ships VST3 + Standalone only.
