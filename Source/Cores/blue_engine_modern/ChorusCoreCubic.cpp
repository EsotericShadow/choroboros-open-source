...
// Global counter to track process calls across instances
int cubicProcessCallCount = 0;
...
void ChorusCoreCubic::prepare(const juce::dsp::ProcessSpec& processSpec)
{
    ...
    // #region agent log - reset process counter
    {
        ...
        prepareCount++;
        cubicProcessCallCount = 0; // Reset on prepare
        std::ofstream logFile("/Users/main/Desktop/green_chorus/.cursor/debug.log", std::ios::app);
        if (logFile.is_open()) {
            logFile << "{\"location\":\"ChorusCoreCubic::prepare\",\"message\":\"Cubic core prepared\",\"data\":{\"prepareCount\":" << prepareCount << "},\"timestamp\":" << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << "}\n";
            logFile.close();
        }
    }
    // #endregion
...
void ChorusCoreCubic::processDelay(ChorusDSP& dsp, juce::dsp::AudioBlock<float>& block, float currentCentreDelayMs)
{
    ...
    cubicProcessCallCount++;
    bool shouldLog = (++logCounter % 100 == 0) || (cubicProcessCallCount <= 10); // Log first 10 calls + every 100th
    ...
    if (shouldLog)
    {
        std::ofstream logFile("/Users/main/Desktop/green_chorus/.cursor/debug.log", std::ios::app);
        if (logFile.is_open()) {
            logFile << "{\"location\":\"ChorusCoreCubic::processDelay\",\"message\":\"Processing audio\",\"data\":{\"core\":\"Blue_Normal_Cubic\",\"delaySamples\":" << sampleDelay << ",\"outputSample\":" << sampleOut << ",\"interpolation\":\"Cubic\"},\"timestamp\":" << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << "}\n";
            logFile.close();
        }
    }
    // #endregion
